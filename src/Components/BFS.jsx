...
export default BFS = () => {
  ...
  useEffect(() => {
    ...
    console.log('hexParametres: ', hexParametres)
    setHexParametres(hexParametres)
    canvasRef.height=height;
    canvasRef.width=width
    console.log('c height: ',canvasRef.height=height);
    console.log('c width: ',canvasRef.width=width)
    drawHexes();
    ...
    //  let rBottomSide = Math.round((height - hexOrigin.y)/(hexHeight/2));
     ...
     for(let row = 0; row <= rBottomSide; row++) {
       ...
       for(let q = -qLeftSide; q <= qRightSide; q++) {
         ...
         if((x > hexWidth/2 && x < width - hexWidth/2) && (y > hexHeight/2 && y < height - hexHeight/2)) {
           drawHex(canvasRef, Point(x,y));
           drawHexCoordinates(canvasRef, Point(x,y), Hex(q-p, row));
         }
       }
     }
     var n = 0;
     for(let r = -1; r >= -rTopSide; r--) {
        ...
        for(let q = -qLeftSide; q <= qRightSide; q++) {
          ...
          if((x > hexWidth/2 && x < width - hexWidth/2) && (y > hexHeight/2 && y < height - hexHeight/2)) {
            drawHex(canvasRef, Point(x,y));
            drawHexCoordinates(canvasRef, Point(x,y), Hex(q+n, r));
          }
        }
     }
   }
...
const getHexParametres = () => {
  ...
  return { hexWidth, hexHeight, vertDist, horizDist }
    ...
    <View ref={ref} style={styles.container}>
      <Canvas ref={canvasRef} height={height} width={width} style={styles.grid} />
    </View>
    )
